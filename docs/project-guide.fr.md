# Companion Gear Upgrades — Guide du projet

> Version française. [English UI tutorial](../README.md) · [English project guide](project-guide.md) · [Tutoriel UI en français](README.fr.md)
...
Pour ajouter un **slot**, mettez à jour simultanément :
...
Le service d’application parcourt déjà `GearPresetOverrides.EditableSlots` : ajouter le slot à ce tableau couvre automatiquement capture, commit et application. Le mapping des types compatibles doit toutefois être ajouté explicitement.

Pour ajouter une **catégorie**, ajoutez aussi :

1. une valeur à `GearPresetCategory` dans [GearPresetConfigTypes.cs](../UI/GearPresetConfigTypes.cs) ;
2. son option dans `LoadTier()` ; les trois catégories actuelles y sont hardcodées ;
3. un cas explicite dans `GetSlotsForCategory()` ; son `default` actuel renvoie les slots Horse ;
4. le brush de catégorie dans `GearCategoryOptionViewModel.GetIconBrush()` ; son `default` utilise le brush des montures.

Oublier un seul de ces points produit souvent une option visible, mais qui n’est pas correctement sauvegardée ou appliquée.
...
Exécutez toutes les commandes de cette section depuis la racine du dépôt.
...
| --- | --- |
| `[CGU] Preset configuration is not ready yet.` | La session de campagne doit avoir atteint `OnSessionLaunched`. Vérifier que le module, Harmony et UIExtenderEx sont chargés, ainsi que la validité du contexte hôte courant. |
...
Le dépôt contient une licence MIT dans [LICENSE.txt](../LICENSE.txt). Ses champs d’attribution (`[year]`, `[fullname]`) sont encore des placeholders et doivent être renseignés avant une distribution publique.
